  ...
  fArmAngle(0.*deg), fArmRotation(0), fSecondArmPhys(0)
{
    ...
    // SetBrickMaterial("G4_Pb");

    // fDetMessenger = new B5DetectorMessenger(this);
}
...
G4VPhysicalVolume* B5DetectorConstruction::Construct(){
    ...
    for (G4int i=0; i<5; ++i)
    {
        position.push_back(G4ThreeVector(15*cm*i,0,0));
    }



    fN = (G4int) position.size();
    ...
    // The solid volume
    G4VSolid* detSolid = new G4Box("detSolid",15*cm,80*cm,4*cm);

    for (G4int i=0; i<G4int(position.size()); ++i)
    {
        // Get current position
        G4ThreeVector pos = position.at(i);

        // Logical Volume Name
        sprintf(char_sdname,"DetectorLogical%d", i);
      	G4String logical_name(char_sdname);

        // Create the logical volume and save into a vector
        G4LogicalVolume* tmp = new G4LogicalVolume(detSolid,scintillator,logical_name);
        ...
        sprintf(char_sdname,"DetectorLogical%d", i);
      	...
      	G4VPhysicalVolume* tmpp = new G4PVPlacement(0,pos,fPaddlesLogical.back(),physical_name,worldLogical, false,i,checkOverlaps);
...
//
    ...
    // Make some sensitive detectors
    G4SDManager* SDman = G4SDManager::GetSDMpointer();
    G4VSensitiveDetector* detSD = new B5ScintillatorSD("/paddle0);
    SDman->AddNewDetector(detSD);
    (fPaddlesLogical.at(i))->SetSensitiveDetector(detSD);
...
//      G4VSensitiveDetector* detSD = new B5ScintillatorSD(SDname);
//      SDman->AddNewDetector(detSD);
//      (fPaddlesLogical.at(i))->SetSensitiveDetector(detSD);
//
//    }
...
//
//    G4Material* uranium = G4Material::GetMaterial("G4_U");
//    G4Material* LEU = G4Material::GetMaterial("LEU");
//    G4Material* HEU = G4Material::GetMaterial("HEU");
//
//    G4Material* C = G4Material::GetMaterial("G4_C");    // 6
//
//    G4Material* Al = G4Material::GetMaterial("G4_Al");  // 13
//    G4Material* K  = G4Material::GetMaterial("G4_K");   // 19
//    G4Material* Ti = G4Material::GetMaterial("G4_Ti");  // 22
//    G4Material* Fe = G4Material::GetMaterial("G4_Fe");  // 26
//    G4Material* Cu = G4Material::GetMaterial("G4_Cu");  // 29
//    G4Material* Se = G4Material::GetMaterial("G4_Se");  // 34
//    G4Material* Sr = G4Material::GetMaterial("G4_Sr");  // 38
//    G4Material* Tc = G4Material::GetMaterial("G4_Tc");  // 43
//    G4Material* Ag = G4Material::GetMaterial("G4_Ag");  // 47
//    G4Material* Sn = G4Material::GetMaterial("G4_Sn");  // 50
//    G4Material* Ce = G4Material::GetMaterial("G4_Ce");  // 58
//    G4Material* Gd = G4Material::GetMaterial("G4_Gd");  // 64
//    G4Material* Er = G4Material::GetMaterial("G4_Er");  // 68
//    G4Material* W  = G4Material::GetMaterial("G4_W");   // 74
//    G4Material* Au = G4Material::GetMaterial("G4_Au");  // 79
//    G4Material* Pb = G4Material::GetMaterial("G4_Pb");  // 82
//
//    G4Material* BrickMat = Er;
//
//    // if(fBrickMatName == "carbon") SetBrickMaterial(carbon);
//    // if(fBrickMatName == "iron") SetBrickMaterial(iron);
//    // if(fBrickMatName == "lead") SetBrickMaterial(lead);
//
//    // Option to switch on/off checking of volumes overlaps
//    //
//    G4bool checkOverlaps = true;
...
//    // Changed name to be more consistent when using different materials
//    G4VSolid* pbSolid = new G4Box("lead_box",fsizex*0.5,fsizey*0.5,0.5*fPbHeight);
//    fPbLogical = new G4LogicalVolume(pbSolid,BrickMat,"pb_bricks_logical");
//    // fPbLogical = new G4LogicalVolume(pbSolid,fBrickMat,"pb_bricks_logical");
//
...
void B5DetectorConstruction::ConstructMaterials()
{
    ...
    G4double nel;       //number of elements

    G4Element* U238 = new G4Element("U238", "U238", z=92, a=238.03*g/mole);
    G4Element* U235 = new G4Element("U235", "U235", z=92, a=235.01*g/mole);

    // U235
    G4Material* LEU = new G4Material("LEU", density=19.1*g/cm3, nel=2);
    LEU->AddElement(U238, 96*perCent);
    LEU->AddElement(U235, 4*perCent);

    // highly enriched uranium
    G4Material* HEU = new G4Material("HEU", density=19.1*g/cm3, nel=2);
    HEU->AddElement(U238, 10*perCent);
    HEU->AddElement(U235, 90*perCent);
...
// void B5DetectorConstruction::SetBrickMaterial(G4String materialChoice)
// {
//   // search the material by its name
//   G4Material* pttoMaterial =
//     G4NistManager::Instance()->FindOrBuildMaterial(materialChoice);

//   if (pttoMaterial && fBrickMat != pttoMaterial) {
//     fBrickMat = pttoMaterial;
//     if(fPbLogical) fPbLogical->SetMaterial(fBrickMat);
